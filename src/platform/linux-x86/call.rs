...
pub unsafe fn arch_prctl(code: i32, arg2: usize) -> Result<(), Errno> {
    ...
    unsafe { syscall2(SYS_ARCH_PRCTL, code, arg2).map(drop) }
}
...
pub unsafe fn fadvise64_64(fd: i32, offset: loff_t, len: loff_t, advice: i32) -> Result<(), Errno> {
    ...
    let advice = advice as usize;
    unsafe { syscall4(SYS_FADVISE64_64, fd, offset, len, advice).map(drop) }
}
...
pub unsafe fn fstatfs64(fd: i32, buf: &mut statfs64_t) -> Result<(), Errno> {
    let fd = fd as usize;
    let buf_ptr = core::ptr::from_mut(buf) as usize;
    unsafe { syscall2(SYS_FSTATFS64, fd, buf_ptr).map(drop) }
}
...
pub unsafe fn ioperm(from: usize, num: usize, turn_on: i32) -> Result<(), Errno> {
    ...
    unsafe { syscall3(SYS_IOPERM, from, num, turn_on).map(drop) }
}
...
pub unsafe fn io_uring_setup(entries: u32, params: &mut io_uring_params_t) -> Result<u32, Errno> {
    ...
    let params_ptr = core::ptr::from_mut(params) as usize;
...
pub unsafe fn lstat64<P: AsRef<Path>>(filename: P, statbuf: &mut stat64_t) -> Result<(), Errno> {
    ...
    let filename_ptr = filename.as_ptr() as usize;
    let statbuf_ptr = core::ptr::from_mut(statbuf) as usize;
    unsafe { syscall2(SYS_LSTAT64, filename_ptr, statbuf_ptr).map(drop) }
}
...
pub unsafe fn memfd_secret(flags: u32) -> Result<i32, Errno> {
    let flags = flags as usize;
    unsafe { syscall1(SYS_MEMFD_SECRET, flags).map(|ret| ret as i32) }
}
...
/// assert!(ret.is_ok());
/// let mut buf = [0_u8; nc::PATH_MAX as usize + 1];
/// let ret = unsafe { nc::readlink(newname, &mut buf) };
/// assert!(ret.is_ok());
/// let n_read = ret.unwrap() as usize;
...
/// assert!(ret.is_ok());
/// ```
pub unsafe fn readlink<P: AsRef<Path>>(filename: P, buf: &mut [u8]) -> Result<ssize_t, Errno> {
    let filename = CString::new(filename.as_ref());
    let filename_ptr = filename.as_ptr() as usize;
    let buf_ptr = buf.as_mut_ptr() as usize;
    let buf_len = buf.len();
...
pub unsafe fn sendfile64(
    ...
    let in_fd = in_fd as usize;
    let offset = offset as usize;
    unsafe { syscall4(SYS_SENDFILE64, out_fd, in_fd, offset, count).map(|ret| ret as ssize_t) }
}
...
/// }
/// ```
pub unsafe fn sendmmsg(sockfd: i32, msgvec: &[mmsghdr_t], flags: i32) -> Result<size_t, Errno> {
    let sockfd = sockfd as usize;
...
pub unsafe fn signal(sig: i32, handler: sighandler_t) -> Result<sighandler_t, Errno> {
    let sig = sig as usize;
    unsafe { syscall2(SYS_SIGNAL, sig, handler).map(|ret| ret as sighandler_t) }
}
...
pub unsafe fn stat64<P: AsRef<Path>>(filename: P, statbuf: &mut stat64_t) -> Result<(), Errno> {
    ...
    let filename_ptr = filename.as_ptr() as usize;
    let statbuf_ptr = core::ptr::from_mut(statbuf) as usize;
    unsafe { syscall2(SYS_STAT64, filename_ptr, statbuf_ptr).map(drop) }
}
...
pub unsafe fn uselib<P: AsRef<Path>>(library: P) -> Result<(), Errno> {
    ...
    unsafe { syscall1(SYS_USELIB, library_ptr).map(drop) }
}
